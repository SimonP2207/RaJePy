...
from RaJePy.maths import physics as phys
...
def rrl_nu_0(atom: str, n: int, delta_n: int = 1) -> float:
    """
    Radio recombination line rest frequency (non-Doppler shifted). Equation
    1.17 of Gordon & Sorochenko.

    :param int n: RRL transition lower electronic level
    :param str atom: Element symbol of RRL's ion
    :param int delta_n: Change in energy levels for transition
                        i.e. n + delta_n -> n (default, 1)
    :return: Rest frequency (Hz)
    :rtype: float
    """
    z = phys.z_number(atom)
    r = phys.rydberg_constant(atom)

    return r * con.c * z ** 2. * (1. / n ** 2. - 1. / (n + delta_n) ** 2.)


def energy_n(n: int, atom: str) -> float:
    """
    Energy of the nth level (erg). Equation 1.14 of Gordon & Sorochenko
    ...
    :rtype: float
    """
    return -2.17989724e-11 * phys.z_number(atom) ** 2. / n ** 2.


def f_n1n2(n_1: int, delta_n: int) -> float:
    """
    Oscillator strength. Equation 2.116 of Gordon & Sorochenko.

    :param int n_1: RRL transition lower electronic level
    :param int delta_n: Change in energy levels for transition
                        i.e. n_1 + delta_n -> n_1
    :return: :math:`f_{n_1,n_2}`
    :rtype: float
    """
    m_deltan = {1: 0.190775,
                2: 0.026332,
    ...
    return n_1 * m_deltan * (1. + 1.5 * delta_n / n_1)


def ni_from_ne(n_e: float, atom='H') -> float:
    """
    Infer ion number density from electron number density assuming standard
    ratios of hydrogen, helium and lithium.
    ...
    :rtype: float
    """
    # Mass ratios from Nieva & Przybilla (2012)
    xyz = {'H': 0.710,
           'He': 0.276,
           'CNO': 0.014}

    mu = (xyz['H'] / phys.atomic_mass("H") * con.u +
          xyz['He'] / phys.atomic_mass("He") * con.u +
          xyz['CNO'] / 14.24) ** -1.

    m_atom = phys.atomic_mass(atom) / con.u

    return xyz[atom] * n_e * mu / m_atom


def deltanu_l(n_e: float, n: int, delta_n: int, gamma: float = 4.5) -> float:
    """
    Line width for purely electron-collisional (Stark), Lorentzian broadening.
    Equation 1 of Smirnov (1985) 2.60 of Gordon & Sorochenko.

    :param float n_e: Electron number density (cm^-3)
    :param int n: RRL transition lower electronic level
    :param int delta_n: Change in energy levels for transition
                        i.e. n + delta_n -> n
    :param float gamma: Growth rate of RRL collisional broadenening as a
                        function of n
    :return: Delta nu_L` (Hz)
    :rtype: float
    """
...
def deltanu_v(fwhm_thermal: float, fwhm_stark: float) -> float:
    """
    Full width at half maximum of the Voigt profile. Sourced from Wikipedia,
    where the proper reference is Olivero & Longbothum (1977)
    ...
    :return: delta nu_V (Hz)
    :rtype: float
    """
    p1 = 0.5346 * fwhm_stark
    p2 = np.sqrt(0.2166 * fwhm_stark ** 2. + fwhm_thermal ** 2.)

    return p1 + p2


def phi_thermal_nu_integrated(freq: float, nu_0: float, fwhm_thermal: float,
                              bw: float) -> float:
    """
    Normalised line profile for purely Doppler-broadened line integrated
    spectrally. From equation 2.20 of Gordon & Sorochenko.
    ...
    :return: phi_G (nu) (Hz^-1)
    :rtype: float
    """

    def indef_integral(nu):
        num = 1.6651092223153954 * fwhm_thermal
        num *= erf(1.6651092223153954 * (nu - nu_0) / fwhm_thermal)
        den = 3.3302184446307908
        return num / den

    result = indef_integral(freq + bw / 2.) - indef_integral(freq - bw / 2.)

    return result


def phi_stark_nu_integrated(freq: float, nu_0: float, fwhm_stark: float,
                            bw: float = 1.) -> float:
    """
    ...
    :param float fwhm_stark: Full-width at half-maximum of the line (Hz) for
                             purely collisional (Stark) broadenening
    :param float bw: Bandwidth of the observation (Hz)
    :return: phi_L (nu) (Hz^-1)
    :rtype: float
    """

    def indef_integral(nu):
        """
        Indefinite integral of Equation 2.30 of Gordon & Sorochenko (2002)
        """
        return -np.arctan(2. * (nu_0 - nu) / fwhm_stark) / np.pi
